; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/dialog_write_image.pro#1 $
;
; Copyright (c) 1998-2006, Research Systems, Inc.  All rights reserved.
...
;	John Armston 2007: rotate/transpose TIFF so image is not horizontally flipped.
;-
;
      ...
      if (ev.done gt 0) then begin
        if (ev.done eq 2) then (*tlbStruct).success=0L else begin
          if ((*tlbStruct).nowrite eq 0) then begin
            ...
            if (writeType EQ 'TIFF') then tImage = reverse(tImage, 3, /overwrite)
...
;------------------------------------------------------------------
function WriteScreenshot, image, red, green, blue, $
