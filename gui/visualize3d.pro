...
;    December 2007 - Capability to have large or small dot sizes (John Armston)
;                  - Capability to subset by return type (John Armston)
;                  - Commented out ENVI dependent features in order to work with IDL VM (John Armston)
;
;###########################################################################
;
; LICENSE
...
PRO GetColor_event, event

  ; This is the event handler for the GetColor routine.  Moving the sliders changes
...
Pro GetColor, colorPtr, oldColor=oldColor, group_leader=group_leader
  ...
  XManager, 'GetColor', colorBase
  
end
;------------------------------------------------------------------------------


PRO GetSubset_event, event

  ; This is the event handler for the GetSubset routine.  Moving the slider changes
    ...
    case event.value of
      0: begin
        ...
        *state.newSubset = subsetUpdate
        widget_control, state.checkID, get_value=typeUpdate
        *state.newType = typeUpdate
      end
...
;if event.id EQ state.checkID then Widget_Control, event.top, Set_UValue=state, /No_Copy
...
Pro GetSubset, newSubset, newType, oldSubset=oldSubset, maxSubset=maxSubset, group_leader=group_leader
  ...
  checkID = cw_bgroup(subsetBase, ['All', 'First', 'Last', 'Ground', 'Non-ground'], /column, space=1, /exclusive, set_value=0, label_top='Which returns?')
...
PRO Data_Open, event
  ...
  FORWARD_FUNCTION Normalize, ReadLAS
  catch, theError
  ...
  cd, file_dirname(inputFiles[0])
  
  
  if (inputFiles[0] ne '') then begin
  
    nFiles = n_elements(inputFiles)
    ReadLAS, inputFiles[0], header, /nodata
    ...
    *info.dataPtr  = 0B
    ...
    if nFiles gt 1 then begin
      ...
      for a=1,nFiles-1 do begin
        ...
        ReadLAS, inputFiles[a], inputHeader, /nodata
    ...
    *info.filesPtr = filesStr
    ...
    Data_Initialize, info, 5e4, type=0
...
PRO Data_Initialize, info, subset, type=type

  ; This procedure initialzes the data object to requested parameters.
  forward_function InitDataLAS, ReadLAS, filterReturns
  ...
  dataStr = InitDataLAS(pointFormat=info.header.pointFormat)
  ...
  for a=0,n_elements(*info.filesPtr)-1 do begin
    ...
    assocLun = 1
    ReadLAS, (*info.filesPtr)[a].name, inputHeader, assocData, assocLun=assocLun
    ...
    for b=0L,nTemp[a]-1 do data[(dStart+b) < (n_elements(data)-1)] = assocData[index[b]]
  ...
  if not keyword_set(type) then begin
    *info.dataPtr = data
    type_str = 'All returns'
  endif else begin
    case type of
      1: begin
        index = filterReturns(data, type=1)
        type_str = 'First returns'
      end
      2: begin
        index = filterReturns(data, type=2)
        type_str = 'Last returns'
      end
      3: begin
        index = filterReturns(data, type=4)
        type_str = 'Ground returns'
      end
      4: begin
        index = filterReturns(data, type=5)
        type_str = 'Non-ground returns'
      end
    endcase
    index = where(index EQ 1, count)
    if (count GT 0) then begin
      *info.dataPtr = data[index]
    endif else begin
      *info.dataPtr = data
    endelse
  endelse
  ...
  widget_control, info.buttons.bType, $
    set_value=type_str
...
PRO Viz_Surface_Elevation_Colors, event

  forward_function XColors
  ...
  CASE thisEvent OF
    ...
    "WIDGET_BUTTON": BEGIN
      ...
      XColors, Group_Leader=event.top, NotifyID=[event.id, event.top], Title="Shading Colors"
    END
...
PRO Viz_Surface_Event, event

  forward_function WriteScreenshot
  ...
  CASE newChange OF
    ...
    'SDOTS':  begin
      info.poly->SetProperty, Style=0, Thick=1
      widget_control, info.buttons.bSDots,  set_button=1
      widget_control, info.buttons.bLDots,  set_button=0
      widget_control, info.buttons.bWire,  set_button=0
      widget_control, info.buttons.bSolid, set_button=0
    end
    'LDOTS':  begin
      info.poly->SetProperty, Style=0, Thick=2
      widget_control, info.buttons.bSDots,  set_button=0
      widget_control, info.buttons.bLDots,  set_button=1
      widget_control, info.buttons.bWire,  set_button=0
      widget_control, info.buttons.bSolid, set_button=0
    end
    'MESH':  begin
    ...
    'SOURCE': begin
      for i=0,n_tags(info.buttons.source)-1 do widget_control, info.buttons.source.(i), set_button=0
      widget_control, info.buttons.source.source, set_button=1
      Viz_Surface_Update, info
    end
    'CLASS': begin
      for i=0,n_tags(info.buttons.source)-1 do widget_control, info.buttons.source.(i), set_button=0
    ...
    'CAPTURE': begin
      ...
      dummy = WriteScreenshot(snapshot, file='QRSC_Lidar_Image.tif')
    end
    ...
    'SUBSET': begin
      ...
      newType = ptr_new(/allocate_heap, /no_copy)
      
      ; Get the subset value.  If it has changed, update the data, then destroy the pointer
      
      GetSubset, newSubset, newType, oldSubset=oldSubset, maxSubset=info.header.nPoints, group_leader=event.top
      ;if *newSubset ne oldSubset then Data_Initialize, info, *newSubset, type = *newType
      Data_Initialize, info, *newSubset, type = *newType
      ptr_free, newSubset, newType
    end
...
PRO Viz_Surface_Update, info
  ...
  if (widget_info(info.buttons.source.inten, /button_set)) then begin
    ...
    info.poly->SetProperty, vert_colors=bytscl((*info.dataPtr).inten, /nan)
    info.thisColorbar->SetProperty, Hide=0
    oTitle->SetProperty, Strings='Intensity'
    oMax = max((*info.dataPtr).inten, min=oMin)
    oTickText->SetProperty, Strings=strcompress(round([oMin,oMax]),/remove)
  endif
  
  if (widget_info(info.buttons.source.source, /button_set)) then begin
    good = where((*info.dataPtr).source ne 65535)
    maxSource = max((*info.dataPtr)[good].source)
    info.poly->SetProperty, vert_colors=bytscl((*info.dataPtr).source, min=0, max=maxSource)
    info.thisColorbar->SetProperty, Hide=0
    oTitle->SetProperty, Strings='Source ID'
    oTickText->SetProperty, Strings=strcompress(string([0,maxSource],format='(i)'),/remove)
  endif
  
  if (widget_info(info.buttons.source.class, /button_set)) then begin
    vMax = max(ishft(ishft((*info.dataPtr).class,4),-4), min=vMin)
    if (vMin eq vMax) then begin
      if (vMax eq 2) then vMin -= 1
      if (vMax lt 2) then vMax += 1
    endif
    info.poly->SetProperty, vert_colors=hist_equal(ishft(ishft((*info.dataPtr).class,4),-4), maxv=vMax, minv=vMin, omin=oMin, omax=oMax)
    info.thisColorbar->SetProperty, Hide=0
    oTitle->SetProperty, Strings='Classification value'
    oTickText->SetProperty, Strings=strcompress(round(long([oMin,oMax])),/remove)
  endif
  
  if (widget_info(info.buttons.source.nReturn, /button_set)) then begin
    vMax = max((*info.dataPtr).nReturn mod 8, min=vMin) > 2
    info.poly->SetProperty, vert_colors=hist_equal((*info.dataPtr).nReturn mod 8, maxv=vMax, minv=vMin, omin=oMin, omax=oMax)
    info.thisColorbar->SetProperty, Hide=0
...
; TODO: Update image feature to handle geoTIFF's instead of ENVI files
...
PRO Viz_Surface_Properties, event
  ...
  CASE newProperty OF
    ...
    'SURFACE_COLOR': BEGIN
      ...
      GetColor, newColor, oldColor=oldColor, group_leader=event.top
...
PRO Viz_Resize, event
  ...
  info.thisColorbar->SetProperty, XCoord_Conv=Normalize([0,255], Position=xPos)
  info.thisColorbar->SetProperty, YCoord_Conv=Normalize([0,14],  Position=yPos)
...
pro Visualize3D, event

  FORWARD_FUNCTION Normalize, InitHeaderLAS
  ...
  thisColorbar->SetProperty, XCoord_Conv=Normalize([0,255], Position=[thisRect[0] + colorbarLocation.x])
  thisColorbar->SetProperty, YCoord_Conv=Normalize([0,15],  Position=[thisRect[1] + colorbarLocation.y])
  ...
  ; surface so the range can be set correctly. Note how the font is set to 8 pt helvetica.
  ...
  tlb    = Widget_Base(Title="Lidar Data Viewer (Modified version of 'Visualize3D' by David Streutker)", Column=1, TLB_Size_Events=1, MBar=menuBase)
  drawID = Widget_Draw(tlb, XSize=800, YSize=600, Graphics_Level=2, Retain=1, Renderer=1, $
    /Expose_Events, /Button_Events, /Wheel_Events, Event_Pro='Viz_Draw_Events')
    
  ; Create FILE menu buttons for opening files and exiting.
    
  fileMenu = Widget_Button(menuBase, Value='File', /Menu)
  
  dummy = Widget_button(fileMenu, Value='Open LAS File(s)', Event_Pro='Data_Open')
  dummy = Widget_button(fileMenu, Value='Save Screenshot', Event_Pro='Viz_Surface_Event', UValue='CAPTURE', /separator)
  dummy = Widget_Button(fileMenu, Value='Exit', Event_Pro='Viz_Surface_Exit', /separator)
  ...
  bSDots  = Widget_Button(surfMenu, Value='Small Dot Surface', Event_Pro='Viz_Surface_Event', UValue='SDOTS', /checked)
  bLDots  = Widget_Button(surfMenu, Value='Large Dot Surface', Event_Pro='Viz_Surface_Event', UValue='LDOTS', /checked)
  bWire  = Widget_Button(surfMenu, Value='Wire Mesh',   Event_Pro='Viz_Surface_Event', UValue='MESH', /checked)
  bSolid = Widget_Button(surfMenu, Value='Solid',       Event_Pro='Viz_Surface_Event', UValue='SOLID', /checked)
  
  colorMenu = Widget_Button(styleMenu, Value='Surface Color Type', /Menu, /Separator)
  
  bSingle = Widget_Button(colorMenu, Value='Single Color',     Event_Pro='Viz_Surface_Event', UValue='SINGLE', /checked)
  bElev   = Widget_Button(colorMenu, Value='Elevation Range',  Event_Pro='Viz_Surface_Event', UValue='RAMPED', /checked)
  bInten  = Widget_Button(colorMenu, Value='Intensity Range',  Event_Pro='Viz_Surface_Event', UValue='INTEN',  /checked)
  bSource = Widget_Button(colorMenu, Value='Source ID',        Event_Pro='Viz_Surface_Event', UValue='SOURCE', /checked)
  bClass  = Widget_Button(colorMenu, Value='Classification',   Event_Pro='Viz_Surface_Event', UValue='CLASS',  /checked)
  bReturn = Widget_Button(colorMenu, Value='Return Number',    Event_Pro='Viz_Surface_Event', UValue='RETURN', /checked)
  bAngle  = Widget_Button(colorMenu, Value='Scan Angle',       Event_Pro='Viz_Surface_Event', UValue='ANGLE',  /checked)
  ;bImage  = Widget_Button(colorMenu, Value='From Image',      Event_Pro='Viz_Surface_Event', UValue='IMAGE',  /checked)
  bRGB    = Widget_Button(colorMenu, Value='RGB (LAS 1.2)',    Event_Pro='Viz_Surface_Event', UValue='RGB',  /checked, sensitive=0)
  
  
  dummy = Widget_Button(styleMenu, Value='Set Surface Color Table', Event_Pro='Viz_Surface_Elevation_Colors')
  dummy = Widget_Button(styleMenu, Value='Set Surface Color',       Event_Pro='Viz_Surface_Properties', $
    UValue='SURFACE_COLOR')
    
  dummy = Widget_Button(styleMenu, Value='Surface Shading ON', /Separator, UValue='Surface Shading OFF', $
    Event_Pro='Viz_Surface_Shading')
  ...
  bDragL = Widget_Button(dragMenu, Value='Low',    Event_Pro='Viz_Surface_Event', UValue='DRAG_LOW', /checked)
  bDragM = Widget_Button(dragMenu, Value='Medium', Event_Pro='Viz_Surface_Event', UValue='DRAG_MEDIUM', /checked)
  bDragH = Widget_Button(dragMenu, Value='High',   Event_Pro='Viz_Surface_Event', UValue='DRAG_HIGH', /checked)
  
  ; Axes and colors.
  
  dummy = Widget_Button(propMenu, Value='Set Label Color',      Event_Pro='Viz_Surface_Properties', UValue='LABEL_COLOR')
  dummy = Widget_Button(propMenu, Value='Set Background Color', Event_Pro='Viz_Surface_Properties', UValue='BACKGROUND_COLOR')
  
  dummy = Widget_Button(propMenu, Value='Axes OFF', /Separator, UValue='Axes ON', Event_Pro='Viz_Labels')
  dummy = Widget_Button(propMenu, Value='Colorbar ON',     UValue='Colorbar OFF', Event_Pro='Viz_Labels')
  ...
  bType = Widget_button(subMenu,  Value='*no data loaded*', sensitive=0)
  dummy   = Widget_button(subMenu,  Value='Change Subset', Event_Pro='Viz_Surface_Event', UValue='SUBSET', /separator)
  
  ; Set the initial button states and create structure with the button IDs
  
  widget_control, bLDots,  set_button=1
  widget_control, bElev,   set_button=1
  widget_control, bDragH,  set_button=1
  
  buttons = { bSDots   : bSDots, $                    ; The small dot surface button.
    bLDots   : bLDots, $                    ; The large dot surface button.
    ...
    source  : bSource, $        ; The source ID color button.
  ...
  XManager, 'Viz_surface', tlb, Cleanup='Viz_Surface_Cleanup', No_Block=1, $
    Event_Handler='Viz_Resize', Group_Leader=groupLeader
