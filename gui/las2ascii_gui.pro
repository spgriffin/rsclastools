...
PRO LAS2Ascii_GUI
  ...
  text1 = WIDGET_LABEL(tlb1, value='LAS Fields to Export', frame=0, /align_center)
  ...
  text = WIDGET_LABEL(Base3, value='The output is a comma-delimited ASCII file.', frame=0, /align_left)
  text = WIDGET_LABEL(Base3, value='Output filename is the input with a ".csv" suffix.', frame=0, /align_left)
  text = WIDGET_LABEL(Base3, value='For example, Input : t1.las', frame=0, /align_left)
  text = WIDGET_LABEL(Base3, value='Output : t1.csv', frame=0, /align_left)
  
  ; Do the rest
  tlb4 = widget_base(tlb, column=1, xsize=!QRSC_LIDAR_XSIZE)
  button = Widget_Button(tlb4, Value='Start Export', UValue='StartLAS2Ascii')
