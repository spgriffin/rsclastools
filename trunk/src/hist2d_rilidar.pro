; $Id: hist_2d.pro,v 1.13 2003/02/03 18:13:16 scottm Exp $
;
...
;   John Armston, November 2003. Now allows the reverse indices to be returned.
;                     Renamed function 'hist_2d_ri'
;   John Armston, January 2006. No bins is now not buffered by one when an integer.
;                               Lines 110-111.
;-

function hist2d_rilidar, im1, im2, reverse_indices, im1bins, im2bins, $
  ...
  im1bins = ceil((max1-min1) / b1)
  im2bins = ceil((max2-min2) / b2)
  ...
  h = HISTOGRAM(h, MIN=0, MAX=ulong64(double(im1bins)*double(im2bins)-1D), reverse_indices=reverse_indices)  ;Get the 1D histogram
